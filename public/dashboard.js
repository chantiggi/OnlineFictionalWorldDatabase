module.exports = function () {
    ...
    function getAuthorInfo(req, res, mysql, context, complete) {
        var inserts = [req.params.userID];
        var query = "SELECT userID, username, CONCAT(firstName, ' ', lastName) AS fullName FROM User "
        + "WHERE userID = ?";
        mysql.pool.query(query, inserts, function (error, results, fields) {
            if(error) {
                console.log(error);
    ...
    router.get('/:userID', function (req, res) {
        ...
        getAuthorInfo(req, res, mysql, context, complete);
        function complete() {
            callbackCount++;
            if (callbackCount >= 2) {
